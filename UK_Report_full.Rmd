...
## Graduation and career-related worry

- The entrance to work life as a major life event [@Ballas2007] 
- Insecurity about finding suitable employment [@Koen2012; @Leach2017]
- Labor market myths [@Ball2018]
- Career construction theory [@Savickas2005]
- career planning as a means to deal with career-related worry [@Stringer2011]

## Career planning [@Gould1979]

  (1) the extent to which career plans exist, 
  (2) how frequently career plans are changed, 
  (3) how clear the plans are, and 
  (4) whether or not a strategy exists for achieving career goals
  
- Career planning has repeatedly been linked to career success and job satisfaction [see @NG2005 for a meta-analysis]
- Positive effect on career adaptability [e.g., @Hirschi2009; @Skorikov2007] as “an individual’s readiness and resources for coping with current and imminent vocational development tasks, occupational transitions, and personal traumas” [@Savickas2005, p. 51]
- Negative effect on career concerns [@Creed2009] 


- worry affects temporal focus on outcomes [@Worthy2014]
- task avoidance theory [Ferrari1995]: (Irrational) fears and self-criticism may increase the tendency to avoid tasks

- affect as information: [@Clore2001]
- worry as a motivator to engage in career planning [@Sweeny2017] 

## Self-efficacy as a mediator? 
- Career construction theory [@Savickas2005]: Career-preparatory actions (adapting) as a source of a sense of mastery 
- Gaining self-efficacy by engaging in career-preparatory behavior [@DelCorso2011]
- The effect of career planning on self-efficacy beliefs remains underexplored
- Career preparatory actions and cognitions as a source of mastery and accomplishment 


- Social cognitive career theory [e.g., @Lent1994]: Self-efficacy as a predictor of a person's engagement in career-preparatory behaviors (like career planning [@Chung2002])

- Bandura: Self-efficacy enahncing positive affect and well-being [@Bandura2000]
- Self-efficacy decreases career-related worries: For example, self-efficacy diminishes pre-task anxiety [@Stumpf1987]


## The research model

- The model variables (assessed at T1 and T2):
  + Self-reported career planning [@Gould1979]
  + Self-efficacy [@Chen2001]
  + Career-related worry [WDQ-R, @VanRijsoort1999] 

# Data Preparation {.tabset}
## Loading data 
```{r loading_packs, eval = FALSE, cache = TRUE, include = F, echo = F}
knitr::opts_chunk$set(include = T, echo = T, warning = F, message = F,  results = "hide")
                      )
ipak <- function(pkg){
...
```
## Attention Check
* Exclude those who fail at least two attention checks
...
* calculate time difference between expected graduation date and time point of assessment (2019-03-13)
``` {r difftime_grad}
...
# add current date as a column and create time distance (time since grad)
my_dat$t1date <- as.Date("2019-03-13")
my_dat$t1date_diff <- as.numeric(difftime(my_dat$t1grad_new, my_dat$t1date), units = "days")
```
...
```
## Mplus data
* can be found as "mplusdat.dat" in Dropbox folder "R_Code"
...
- Four-wave field study with 299 British students during the pre-graduation phase
- T1: Three months before graduation; T2: six weeks before graduation

# Sample characteristics {.tabset}
```{r sample_chars,  warning = F, message = F}
mydat_multi <- my_dat %>% 
  zap_label(.) %>%
  zap_labels(.) %>%
...
descr <- mydat_multi %>%
  ...
  summarise("Percent Female" = ((mean(mydat_multi$t1sex, na.rm = T)-1)*100), "Mean Age" = mean(mydat_multi$t1age, na.rm = T), "Percent English-speaking" = (2-mean(my_dat$t1lang, na.rm = T))*100, "Percent job found at T2" = ((mean(mydat_multi$t2job, na.rm = T)-1)*100)) %>% 
  round(., digits = 2)
...
demos <- mydat_multi[, c(120, 121, 122, 133)]
...
corstar <- data.frame(corstars(comp_df, removeTriangle = "upper", result="none"))

kable(corstar, align = "c", escape = F) %>%
    kable_styling(full_width = F)

corstar1 <- corstar
names(corstar1)[1] <- paste0(names(corstar1)[1], 
                                footnote_marker_alphabet(1, "html"))
names(corstar1)[3] <- paste0(names(corstar1)[3], 
                                footnote_marker_alphabet(2, "html"))
names(corstar1)[4] <- paste0(names(corstar1)[4], 
                                footnote_marker_alphabet(3, "html"))

row.names(corstar1)[1] <- paste0(row.names(corstar1)[1], 
                                footnote_marker_alphabet(1, "html"))
row.names(corstar1)[3] <- paste0(row.names(corstar1)[3], 
                                footnote_marker_alphabet(2, "html"))
row.names(corstar1)[4] <- paste0(row.names(corstar1)[4], 
                                footnote_marker_alphabet(3, "html"))
```
...
``` {r corr_table2,  echo = FALSE}
kable(corstar1, "html", align = "c", escape = F) %>%
    kable_styling(full_width = F) %>%
  footnote(general = "Correlations based on pairwise complete observations; N = 219-299",
           alphabet =c("1 = male, 2 = female","1 = English, 2 = other", "1 = 'No' or 'No, I will not graduate this summer', 2 = 'Yes' or 
                       'Potentially (e.g., I have a received an informal job offer)'"),
           footnote_as_chunk = T)
```
...
## One factor model
factor_analysis_df<- dplyr::select(mplus_dat, matches("t1cpl_|t1se_|t1wor_"))
...
```{r display_CFA, message = FALSE, warnings = FALSE}
library(kableExtra)
...
t1wor ~~ t2wor
'
fit_configural_ALL <- lavaan::cfa(configural_ALL, data = mplus_dat, meanstructure = TRUE, missing="fiml")
...
```{r display_mi_and_mi_comparison}
library(kableExtra)
...
fit_BLCS_Cplan_worry <- lavaan(BLCS_Cplan_worry, data=my_dat, estimator='mlr',fixed.x=FALSE,missing='fiml')
...
dCplan_t1 ~~ dWorry_t1
'
fitBLCS_Cplan_byworryfix <- lavaan(BLCS_Cplan_byworryfix, data=my_dat, estimator='mlr',fixed.x=FALSE,missing='fiml')
...
```{r display_comp_cplan_worryfixed}
...
'
fitBLCS_worry_bycplfixed <- lavaan(BLCS_worry_bycplfixed, data=my_dat, estimator='mlr',fixed.x=FALSE,missing='fiml')
summary(fitBLCS_worry_bycplfixed, fit.measures=TRUE, standardized=TRUE, rsquare=TRUE)
```

```{r comp_worry_cplfixed}
# anova
...
'
fit_BLCS_Cplan_worry_1 <- lavaan(BLCS_Cplan_worry_1, data=my_dat, estimator='mlr',fixed.x=FALSE,missing='fiml')
...
fit_BLCS_Cplan_worry_2 <- lavaan(BLCS_Cplan_worry_2, data=my_dat, estimator='mlr',fixed.x=FALSE,missing='fiml')
...
fit_BLCS_se_cpl <- lavaan(BLCS_se_cpl, data=my_dat, estimator='mlr',fixed.x=FALSE,missing='fiml')
...
fit_BLCS_cplan_sefixed <- lavaan(BLCS_cplan_sefixed, data=my_dat, estimator='mlr',fixed.x=FALSE,missing='fiml')
summary(fit_BLCS_cplan_sefixed, fit.measures=TRUE, standardized=TRUE, rsquare=TRUE)
```

```{r comp_BLCS_cplan_sefixed}
# anova
...
fit_BLCS_se_cplanfixed <- lavaan(BLCS_se_cplanfixed, data=my_dat, estimator='mlr',fixed.x=FALSE,missing='fiml')
summary(fit_BLCS_se_cplanfixed, fit.measures=TRUE, standardized=TRUE, rsquare=TRUE)
```

```{r comp_BLCS_se_cplanfixed}
# anova
...
fit_BLCS_se_worry <- lavaan(BLCS_se_worry, data=my_dat, estimator='mlr',fixed.x=FALSE,missing='fiml')
...
dSE_T1 ~~ dWorry_T1
'
fit_BLCS_se_worryfixed <- lavaan(BLCS_se_worryfixed, data=my_dat, estimator='mlr',fixed.x=FALSE,missing='fiml')
summary(fit_BLCS_se_worryfixed, fit.measures=TRUE, standardized=TRUE, rsquare=TRUE)
```

```{r comp_BLCS_se_worryfixed}
# anova
...
fit_BLCS_worry_sefixed <- lavaan(BLCS_worry_sefixed, data=my_dat, estimator='mlr',fixed.x=FALSE,missing='fiml')
summary(fit_BLCS_worry_sefixed, fit.measures=TRUE, standardized=TRUE, rsquare=TRUE)
```

```{r comp_BLCS_worry_sefixed}
# anova
...
```{r display_BLCS_Cplan_worry_t2jobfound}
library(kableExtra)
...
# Mediation 
## The effect of T1 Career planning on T2 Worry 'mediated' by T2 Self-efficacy
```{r med_cpl_se_wo}
t1cplan_t2se_t2worry <- '
# measurement model:
t1cpl =~ t1cpl_1+t1cpl_2+t1cpl_3+t1cpl_4+t1cpl_5+t1cpl_6
t2se =~ t2se_1+t2se_2+t2se_3+t2se_4+t2se_5+t2se_6+t2se_7+t2se_8 
t2wor =~ t2wor_1 + t2wor_2 + t2wor_3+ t2wor_4

t2wor ~ b*t2se + c* t1cpl
t2se ~ a*t1cpl 
indirect := a * b
total    := c + (a * b)
'
set.seed(1234)
fit_t1cplan_t2se_t2worry <- lavaan::sem(model = t1cplan_t2se_t2worry, data = my_dat, se = "bootstrap", bootstrap = 1000, missing = "fiml")
summary(fit_t1cplan_t2se_t2worry, fit.measures=TRUE, standardize=F, rsquare=TRUE, 
        estimates = TRUE, ci = TRUE)

library(lavaan)
parameterEstimates(fit_t1cplan_t2se_t2worry)
paras_fit_t1cplan_t2se_t2worry<- parameterEstimates(fit_t1cplan_t2se_t2worry)[c(64,65), ]
paras_fit_t1cplan_t2se_t2worry$label <- NULL
paras_fit_t1cplan_t2se_t2worry[, 7] <- format.pval(paras_fit_t1cplan_t2se_t2worry[ , 7], eps = .001, digits = 3)
rownames(paras_fit_t1cplan_t2se_t2worry) <- NULL

library(glue)
BLCS_Cplan_worry_meas <- generate_cfa_fitmeasures(fit_t1cplan_t2se_t2worry)
for (i in seq_along(BLCS_Cplan_worry_meas)) {
    if (is.numeric(i)) {
      round(i, 3)
    }
...
kableExtra::kable(paras_fit_t1cplan_t2se_t2worry, booktabs = TRUE, digits = 3, format = 'html', escape = FALSE, caption = "The effect of career planning on worry mediate' by self-efficacy: Inirect and total effect") %>%
  kable_styling(position = 'left', full_width = F) 
```

```{r display_med_cpl_se_wo_path}
lyot <- matrix(c(-0.5, 0.0, 0, 0.5, 0.5, 0), ncol = 2, byrow = T)
semPaths(fit_t1cplan_t2se_t2worry, what = "est", layout = lyot, intercepts = FALSE, residuals = FALSE, structural = TRUE, sizeMan = 4, sizeLat = 12, nCharNodes = 0, edge.label.cex = 1.6, fixedStyle = c("grey",3), curvePivot = TRUE)
```
